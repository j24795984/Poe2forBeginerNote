...
type GuideBlock =
	...
	| { type: 'subheading'; text: string; url?: string }
	| { type: 'list'; ordered: boolean; items: string[]; layout?: 'horizontal' }
	| { type: 'table'; headers: string[]; rows: string[][]; columns?: string }
	| { type: 'links'; items: GuideLink[] }
	| { type: 'feature'; title: string; description: string; image: string; imageAlt: string };
type MechanicGuideSection = { id: string; number?: string; title: string; titleUrl?: string; blocks: GuideBlock[] };
type MechanicGuide = {
	id: string;
	mechanicId: string;
	name: string;
	...
	versionNote: string;
	summary: string;
	sections: MechanicGuideSection[];
	sources: Array<GuideLink & { id: string }>;
};
const props = defineProps<{ initialData: AtlasData; initialMechanicGuides: MechanicGuide[] }>();
...
const mechanicGuides = computed(() => new Map(props.initialMechanicGuides.map((guide) => [guide.mechanicId, guide])));
const inlineIntroSection = computed(() => selectedCategory.value?.slug === 'league-mechanics' ? selectedCategory.value.sections[0] : undefined);
const cardSections = computed(() => inlineIntroSection.value ? selectedCategory.value?.sections.slice(1) ?? [] : selectedCategory.value?.sections ?? []);

function guideAssetUrl(path: string) {
	...
	<div v-else-if="selectedCategory" class="atlas-shell">
		<div class="atlas-workspace">
			...
			<section ref="detailPanel" class="atlas-detail atlas-detail-motion" aria-live="polite">
				<header class="atlas-intel-copy mb-7 grid gap-3 px-1 md:mb-9"><p class="atlas-accent-bright-text font-english-decorative text-xs font-semibold">ATLAS INTEL</p><h2 class="font-english-body text-3xl font-bold text-stone-100 [text-wrap:pretty] md:text-4xl">{{ selectedCategory.name }}</h2><p class="max-w-3xl text-base leading-8 text-stone-100 md:text-lg">{{ selectedCategory.summary }}</p><section v-if="inlineIntroSection" class="mt-3"><h3 class="atlas-accent-text font-english-body text-xl font-semibold">{{ inlineIntroSection.title }}</h3><p v-if="inlineIntroSection.description" class="mt-3 max-w-4xl leading-8 text-stone-300">{{ inlineIntroSection.description }}</p><ul v-if="inlineIntroSection.bullets" class="mt-3 grid gap-2 pl-5 text-stone-200"><li v-for="item in inlineIntroSection.bullets" :key="item.id" class="leading-7">{{ item.text }}</li></ul></section></header>
				<ul class="m-0 grid list-none gap-7 p-0 md:gap-8">
					<li v-for="section in cardSections" :key="section.id" class="atlas-content-card rounded-[8px] border border-stone-100/15 px-6 py-8 shadow-xl shadow-black/25 md:px-10 md:py-10"><span class="atlas-content-card-ornament" :style="{ '--atlas-card-ornament': `url('${cardOrnamentUrl}')` }" aria-hidden="true"></span><article><h3 class="atlas-accent-text font-english-body text-2xl font-semibold tracking-[-.02em] md:text-3xl">{{ section.title }}</h3><p v-if="section.description" class="mt-4 max-w-4xl leading-8 text-stone-300">{{ section.description }}</p><ul v-if="section.bullets" class="atlas-accent-border-35 mt-6 grid gap-3 border-l pl-5 text-stone-300 md:max-w-4xl"><li v-for="item in section.bullets" :key="item.id" class="leading-7">{{ item.text }}</li></ul><ol v-if="section.steps" class="mt-6 grid gap-4 md:max-w-4xl"><li v-for="(step, index) in section.steps" :key="step.id" class="grid grid-cols-[2rem_1fr] gap-3 leading-7 text-stone-300"><span class="atlas-accent-text font-english-body text-lg font-bold">0{{ index + 1 }}</span><span>{{ step.text }}</span></li></ol><div v-if="section.table" class="atlas-data-grid mt-6 text-sm" :style="{ '--atlas-grid-columns': section.table.headers.length }" role="table"><div class="atlas-data-grid-row atlas-data-grid-header" role="row"><div v-for="header in section.table.headers" :key="header" role="columnheader">{{ header }}</div></div><div v-for="(row, rowIndex) in section.table.rows" :key="rowIndex" class="atlas-data-grid-row" role="row"><div v-for="(cell, cellIndex) in row" :key="cellIndex" role="cell">{{ cell }}</div></div></div></article></li>
					<li v-if="selectedMechanics.length" ref="leagueMechanicsCard" class="atlas-content-card atlas-league-mechanics-card rounded-[8px] border border-stone-100/15 px-6 py-8 shadow-xl shadow-black/25 md:px-10 md:py-10"><span class="atlas-content-card-ornament" :style="{ '--atlas-card-ornament': `url('${cardOrnamentUrl}')` }" aria-hidden="true"></span><section class="atlas-league-mechanics-panel" aria-labelledby="league-mechanics-heading"><h3 id="league-mechanics-heading" class="atlas-accent-text font-english-body text-2xl font-semibold">目前聯盟機制</h3><nav class="content-quick-nav mt-6" aria-label="聯盟機制快速導覽"><ul class="content-quick-nav-list m-0 list-none p-0"><li v-for="mechanic in selectedMechanics" :key="mechanic.id"><button class="content-quick-nav-button" type="button" @click="scrollToMechanic(mechanic.id)">{{ mechanic.name }}</button></li></ul></nav><div ref="leagueMechanicsViewport" class="atlas-league-mechanic-list mt-6"><div ref="leagueMechanicsTrack" class="atlas-league-mechanic-track"><article v-for="mechanic in selectedMechanics" :id="`league-mechanic-${mechanic.id}`" :key="mechanic.id" :ref="(element) => { if (element) mechanicElements.set(mechanic.id, element as HTMLElement); else mechanicElements.delete(mechanic.id); }" class="atlas-league-mechanic"><header><h4 class="atlas-accent-text font-english-body text-xl font-semibold">{{ mechanic.name }}</h4><p class="mt-3 leading-7 text-stone-300">{{ mechanicGuides.get(mechanic.id)?.summary ?? mechanic.summary }}</p><p v-if="mechanicGuides.get(mechanic.id)?.versionNote" class="atlas-mechanic-version mt-4">{{ mechanicGuides.get(mechanic.id)?.versionNote }}</p></header><div v-if="mechanicGuides.get(mechanic.id)" class="atlas-mechanic-guide"><section v-for="section in mechanicGuides.get(mechanic.id)?.sections" :key="section.id" class="atlas-mechanic-guide-section"><div class="atlas-mechanic-section-content"><h5><a v-if="section.titleUrl" :href="section.titleUrl" target="_blank" rel="noreferrer">{{ section.title }}</a><template v-else>{{ section.title }}</template></h5><template v-for="(block, blockIndex) in section.blocks" :key="`${section.id}-${blockIndex}`"><p v-if="block.type === 'paragraph'" class="atlas-mechanic-paragraph">{{ block.text }}</p><p v-else-if="block.type === 'callout'" class="atlas-mechanic-callout">{{ block.text }}</p><h6 v-else-if="block.type === 'subheading'" class="atlas-mechanic-subheading"><a v-if="block.url" :href="block.url" target="_blank" rel="noreferrer">{{ block.text }}</a><template v-else>{{ block.text }}</template></h6><component :is="block.ordered ? 'ol' : 'ul'" v-else-if="block.type === 'list'" class="atlas-mechanic-list" :class="{ 'atlas-mechanic-list-ordered': block.ordered, 'atlas-mechanic-list-horizontal': block.layout === 'horizontal' }"><li v-for="item in block.items" :key="item">{{ item }}</li></component><div v-else-if="block.type === 'table'" class="atlas-data-grid atlas-mechanic-table" :style="{ '--atlas-grid-columns': block.headers.length, '--atlas-grid-template': block.columns }" role="table"><div class="atlas-data-grid-row atlas-data-grid-header" role="row"><div v-for="header in block.headers" :key="header" role="columnheader">{{ header }}</div></div><div v-for="(row, rowIndex) in block.rows" :key="rowIndex" class="atlas-data-grid-row" role="row"><div v-for="(cell, cellIndex) in row" :key="cellIndex" role="cell">{{ cell }}</div></div></div><ul v-else-if="block.type === 'links'" class="atlas-mechanic-links"><li v-for="link in block.items" :key="link.label"><a v-if="link.url" :href="link.url" target="_blank" rel="noreferrer">{{ link.label }}</a><span v-else>{{ link.label }}</span></li></ul><div v-else-if="block.type === 'feature'" class="atlas-mechanic-feature"><img :src="guideAssetUrl(block.image)" :alt="block.imageAlt" width="64" height="64" loading="lazy"><div><strong>{{ block.title }}</strong><p>{{ block.description }}</p></div></div></template></div></section></div></article></div></div></section></li>
				</ul>
			</section>
		</div>
	</div>
