...
- Initial currency, unique, and mechanic-item data is loaded during the Astro build and passed into the Vue island as initial props.
- The page uses one value-sorted list with a sticky category/filter surface above it. Primary categories are currency, unique equipment (including jewels), and fragments/mechanic items.
- Desktop uses a five-column grid for item name, drop area, related boss, approximate Divine Orb value, and acquisition notes. At narrower widths each grid row becomes a labelled card so no horizontal table scrolling is required.
- Item names link to their corresponding PoE2DB pages. Currency names intentionally share the currency economy page.
- Unknown or non-specific drop areas and bosses are shown as `無`; the interface must not infer an unsupported specific drop source.
