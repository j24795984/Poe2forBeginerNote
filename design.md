...
- Campaign data: `public/data/campaign.json` supplies the five fixed chapter categories (`第一章` through `第四章`, plus `間歇`), all chapter map references (`act-1-1.webp` through `act-5.webp`), optional compact map-tab labels, and normalized named map-point coordinates used by reward-card positioning. Third chapter exposes four map states: present surface, present underground, past surface, and past underground. `public/data/poe2_campaign_permanent_rewards_by_map_0.5_zh-TW.json` supplies the permanent-reward entries rendered by the campaign page; all initial data is loaded during the Astro build and passed to the Vue island.
