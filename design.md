...
- Detailed league-mechanic guides are stored separately from the atlas presentation summary. `public/data/atlas-mechanic-abyss.json` and `public/data/atlas-mechanic-breach.json` preserve the corresponding Markdown guides as structured sections, blocks, tables, inline links, and optional image-feature rows. `src/pages/atlas/index.astro` loads both documents at build time and passes them to `AtlasCategoryTabs.vue` as initial props, so `/atlas/#league-mechanics` renders the first usable guide content in static HTML. The league-mechanics category's first presentation section (`機制選擇`) is rendered without a card inside the upper intel heading stack, directly after the category summary; later sections retain the normal card loop. Guide sections do not display side numbers or a reference footer; source records remain in JSON for traceability, while the first relevant in-body term carries the external link. Individual mechanic articles form one flat, unframed sequence without article-level inset padding; the enclosing league-mechanics card remains the sole visual surface. Two-column guide tables may set a narrower label column and retain subgrid alignment, while short name lists may opt into a wrapping horizontal flex row. The existing mechanism quick navigation, pinned overflow track, gold hierarchy, and reduced-motion contracts remain unchanged.
